...
$slot4 =         @( 1187, 1000 )
$slot5 =         @( 1384, 1000 )
$buttonConfig =  @( 1904, 886 )
$buttonSurrend = @( 767 , 845 )
$buttonConfirm = @( 835 , 485 )
$buttonCancel =  @( 1289, 846 )
$battlefield1 =  @( 1306, 651 )
$battlefield2 =  @( 1304, 237 )
$battlefield3 =  @( 598 , 240 )
$battlefield4 =  @( 1306, 651 )
...
param($targetX, $targetY, $target)
for($i=100;$i -ge 1;$i--){
$Position = [system.windows.forms.cursor]::Position
$PositionChangeX = ($targetX - $Position.x)/$i
$PositionChangeY = ($targetY - $Position.y)/$i

[system.windows.forms.cursor]::Position = New-Object system.drawing.point(($Position.x + $PositionChangeX), ($Position.y + $PositionChangeY))

if($i -ge 2){start-sleep -m 5}
}
start-sleep -m 20
}
#use case:
...
:skipAll while ($roundCount -le $inputRounds){
	...
	if ($processClient){
        ...
        while($timesup.elapsed -lt $timeout){
            ...
            if(!($processGame)){Break}
            #buy chessman
            moveCursorToSpot -targetX $slot1[0] -targetY $slot1[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -m 200
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $slot2[0] -targetY $slot2[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -m 200
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $slot3[0] -targetY $slot3[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -m 200
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $slot4[0] -targetY $slot4[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -m 200
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $slot5[0] -targetY $slot5[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -s 1
            ...
            if(!($processGame)){Break}
            #collect loot
            moveCursorToSpot -targetX $battlefield1[0] -targetY $battlefield1[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonRight
            start-sleep -s 5
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $battlefield2[0] -targetY $battlefield2[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonRight
            start-sleep -s 5
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $battlefield3[0] -targetY $battlefield3[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonRight
            start-sleep -s 5
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $battlefield4[0] -targetY $battlefield4[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonRight
            start-sleep -s 1
        ...
        while($true){
            #surrender
            moveCursorToSpot -targetX $buttonConfig[0] -targetY $buttonConfig[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -s 1
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $buttonSurrend[0] -targetY $buttonSurrend[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -s 1
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $buttonConfirm[0] -targetY $buttonConfirm[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -s 1
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            moveCursorToSpot -targetX $buttonCancel[0] -targetY $buttonCancel[1]
            $processGame = Get-Process | Where-Object {$_.ProcessName -eq $targetGame}
            if(!($processGame)){Break}
            Click-MouseButtonLeft
            start-sleep -s 1
